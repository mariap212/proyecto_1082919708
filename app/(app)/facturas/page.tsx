...
export default function FacturasPage() {
  const [invs, setInvs] = useState<Invoice[] | null>(null);
  const [includeVoided, setIncludeVoided] = useState(true);
  const [refresh, setRefresh] = useState(0);

  useEffect(() => {
    setInvs(null);
    apiGet<Invoice[]>(`/api/invoices?include_voided=${includeVoided}`)
      .then(setInvs)
      .catch(() => setInvs([]));
    ...
    <>
      ...
      {invs === null ? (
        ...
        <EmptyState
          ...
          description="Las facturas se generan automáticamente al aprobar pedidos."
        />
      ) : (
        <Panel padded={false}>
